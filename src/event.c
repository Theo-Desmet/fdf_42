...
/*   Updated: 2022/03/21 14:38:02 by tdesmet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_hook(int keycode, t_data *data)
{
	...
	else if (keycode == KEY_S_LOW || keycode == KEY_S_UPP)
	{
		...
		data->color = 0;
	return (ft_hook_move(keycode, data));
}

int	ft_hook_move(int keycode, t_data *data)
{
	if (data->pos_y / 100 > -1 * data->win_height && keycode == KEY_UP)
		data->pos_y -= data->win_height / 50 + 1;
	else if (data->pos_y / 100 < data->win_height && keycode == KEY_DOWN)
		data->pos_y += data->win_height / 50 - 1;
	else if (data->pos_x / 100 > -1 * data->win_wide && keycode == KEY_LEFT)
		data->pos_x -= data->win_wide / 50;
	else if (data->pos_x / 100 < data->win_wide && keycode == KEY_RIGHT)
		data->pos_x += data->win_wide / 50;
	else if (data->pos_z > 1 && (keycode == KEY_Q_UPP || keycode == KEY_Q_LOW))
		data->pos_z -= 1;
	else if (data->pos_z < 99 && (keycode == KEY_E_UPP || keycode == KEY_E_LOW))
		data->pos_z += 1;
	else if (data->zoom > 1 && (keycode == KEY_R_LOW || keycode == KEY_R_UPP))
		data->zoom -= data->zoom / 5 + 1;
	else if (keycode == KEY_F_LOW || keycode == KEY_F_UPP)
		data->zoom += 1 + data->zoom / 5 + 1;
	return (ft_hook_project(keycode, data));
}
